 ...
 * var myLinkExtractor = linkExtractor.create({
 *   siteRoot: 'http://my-domain.com',
 *   debug: true 
 * }).getLinks()...
 */
exports.create = function (options) {
  return new LinkExtractor(options);
};
...
var LinkExtractor = function (options) {
    ...
    this.avoidUrl = _.isArray(options.avoidUrl) ? options.avoidUrl : [];
    return this;
};
...
/** @type {Array} Links array */
LinkExtractor.prototype.filter = false;
        ...
        function (errors, window) {
            ...
            _.forEach(links, function (v) {
                ...
                if (_.isString(stringLink) && stringLink !== "" && // Valid URL
                    ...
                    ( (_.isFunction(self.filter) && self.filter(stringLink)) || self.filter === false) && // Check if filter exist and pass!
...
LinkExtractor.prototype.parseArray = function () {
    ...
    if (parsedLink === this.links.length) {
        this.debug('Finish!');
        this.linksPromise.resolve(_.map(this.links, 'url'));
        this.destroy();
    }
};
